...
class SignalWarningExtractor:
    ...
    def extract_signals(self, text: str, pdf_path: Optional[str] = None) -> Dict[int, Dict[str, Optional[str]]]:
        ...
        if pdf_path:
            try:
                ...
                print(f"[WARNING] Cannot access PDF file, falling back to text parsing: {e}", file=sys.stderr)
            except Exception as e:
                # Fall back to text-based extraction if table extraction fails
                print(f"[WARNING] Table extraction failed, falling back to text parsing: {e}", file=sys.stderr)
    ...
    def _extract_signals_from_table(self, pdf_path: str) -> Optional[Dict[int, Dict[str, Optional[str]]]]:
        ...
        try:
                    ...
                    for table in tables:
                        ...
                        for i, row in enumerate(table[: self.MAX_HEADER_SEARCH_ROWS]):
                            if row and len(row) >= self.MIN_COLUMNS_FOR_TCWS_TABLE:
                                ...
                                if (
                                    "tcws" in row_str
                                    and "luzon" in row_str
                                    and "visayas" in row_str
            ...
            print(f"[ERROR] Error accessing PDF file: {e}", file=sys.stderr)
            return None
        except Exception as e:
            print(f"[ERROR] Error extracting table from PDF: {e}", file=sys.stderr)
            return None
    ...
    def _parse_signal_table(self, table_section: str) -> Dict[int, Dict[str, Optional[str]]]:
        ...
        """
        result = {
            1: {"Luzon": None, "Visayas": None, "Mindanao": None, "Other": None},
    ...
    def _filter_impact_descriptions_from_location_lines(self, raw_lines: List[str]) -> List[str]:
        ...
        for line_text in raw_lines:
            ...
            if line_lower.startswith("(") and any(
                kw in line_lower for kw in ["strong winds", "prevailing", "expected"]
            ):
                ...
                location_match = re.search(
                    r"(?:portion of|area of|island of|islands of|province of|city of|municipality of|municipalities of|barangay of|barangays of|northern|southern|eastern|western)\s+(.+)",
                    line_text,
                    re.IGNORECASE,
                )
                if location_match:
            ...
            if impact_description_mode:
                ...
                if any(
                    loc in line_text
                    for loc in ["Island", "Islands", "Province", "City", "Municipality", "Barangay", "Region"]
                ):
                    impact_description_mode = False
                ...
                # Skip if this looks like impact text continuation
                    ...
                    kw in line_lower
                    for kw in ["prevailing", "expected", "within", "hours", "threat", "property", "danger", "damage"]
                ):
                    continue
                else:
    ...
    def _parse_locations_with_islands(self, location_text: str, full_section: str) -> Dict[str, Optional[str]]:
        ...
        for location_part in location_parts:
            ...
            if not island and any(
                vague in main_location.lower()
                for vague in ["most of", "northeastern", "northwestern", "southeastern", "southwestern"]
            ):
...
class TyphoonLocationExtractor:
    """Extracts typhoon location from bulletin"""

    pass


class TyphoonMovementExtractor:
    """Extracts typhoon movement from bulletin"""

    pass


class TyphoonWindspeedExtractor:
    """Extracts typhoon windspeed from bulletin"""
...
class TyphoonBulletinExtractor:
    ...
    def extract_from_pdf(self, pdf_path: str) -> Dict:
        ...
        try:
            ...
            print(f"[ERROR] Error reading PDF {pdf_path}: {e}", file=sys.stderr)
            return None
                ...
                if i + 1 < len(lines):
                    ...
                    if match:
                        ...
                        name_match = re.search(
                            r'[\u201c\u201d""]([A-Z][A-Za-z]*)[\u201c\u201d""]', full_name, re.IGNORECASE
                        )
                        if name_match:
    ...
    def _extract_typhoon_location(self, text: str) -> str:
        ...
        if match:
            ...
            location_match = re.search(
                r"(\d{1,2},?\d{3}\s+km\s+(?:East|West|North|South|Northeast|Northwest|Southeast|Southwest)[^(]*?)(?:\(|°|$)",
                location_block,
                re.IGNORECASE,
            )
            if location_match:
            ...
            location_match = re.search(
                r"(\d+\s+km\s+(?:East|West|North|South|Northeast|Northwest|Southeast|Southwest)[^(]*?)(?:\(|°|$)",
                location_block,
                re.IGNORECASE,
            )
            if location_match:
    ...
    def _extract_typhoon_movement(self, text: str) -> str:
        ...
        if match:
            ...
            movement_match = re.search(
                r"((?:(?:North|South|East|West|North-?East|North-?West|South-?East|South-?West)\s+)*(?:Northwestward|Northeastward|Southwestward|Southeastward|Northward|Southward|Eastward|Westward)\s+at\s+\d+\s+km/h)",
                movement_block,
                re.IGNORECASE,
            )
            if movement_match:
    ...
    def _extract_typhoon_windspeed(self, text: str) -> str:
        ...
        if match:
            ...
            wind_match = re.search(
                r"(Maximum\s+sustained\s+wind[s]?\s+of\s+\d+\s+km/h[^.]*(?:and central pressure[^.]*)?)",
                intensity_block,
                re.IGNORECASE,
            )
            if wind_match:
