#!/usr/bin/env python
"""
PAGASA Weather Advisory Extractor (Hybrid PDF/HTML) - Modular Library Version
...
This is a pure library module designed to be imported and used by other scripts.
No CLI functionality - use scrape_and_extract() as the main entry point.

Example:
    from modular.advisory_scraper import scrape_and_extract, extract_from_url
    
    # Extract from live PAGASA URL
...
class RainfallAdvisoryExtractor:
    ...
    def _load_consolidated_locations(self) -> Set[str]:
        ...
        if not CONSOLIDATED_LOCATIONS_PATH.exists():
            print(f"[WARNING] Consolidated locations file not found: {CONSOLIDATED_LOCATIONS_PATH}", file=sys.stderr)
            return locations
        ...
        try:
            ...
            print(f"[WARNING] Failed to load consolidated locations: {e}", file=sys.stderr)
    ...
    def extract_rainfall_tables_from_pdf(self, pdf_path: str) -> List[List[List]]:
        ...
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if not has_text:
                    print("[WARNING] PDF is image-based (scanned document) with no extractable text", file=sys.stderr)
                    return []
    ...
    def extract_rainfall_warnings_from_pdf(self, pdf_path: str) -> Dict:
        ...
        if not tables:
            print("[WARNING] No rainfall tables found in PDF or PDF is image-based", file=sys.stderr)
    ...
    def extract_advisory_text_from_html(self, html_content: str) -> Optional[str]:
        ...
        if not advisory_div:
            print("[WARNING] Could not find weekly-content-adv div", file=sys.stderr)
            return None
        ...
        print("[WARNING] No rainfall advisory text found in HTML", file=sys.stderr)
        return None
    ...
    def extract_rainfall_warnings_from_html(self, url: str) -> Dict:
        ...
        if not advisory_text:
            print("[WARNING] No advisory text found", file=sys.stderr)
...
def cleanup_temp_pdf(pdf_path):
    ...
    try:
        ...
        print(f"[WARNING] Failed to delete temporary PDF: {e}", file=sys.stderr)
...
def extract_pdf_url_from_page(page_url):
    ...
    try:
        ...
        print("[WARNING] No PDF URL found on page", file=sys.stderr)
        return None
        
    except Exception as e:
        print(f"[ERROR] Failed to extract PDF URL: {e}", file=sys.stderr)
        return None
...
def extract_from_url(url: str) -> Dict:
    ...
    Args:
        url: URL to extract from (PAGASA page, direct PDF, or HTML file)
...
def scrape_and_extract():
    """
    Scrape and extract from live PAGASA URL.
    
    This is the main entry point for extracting data from the current PAGASA weather advisory.
